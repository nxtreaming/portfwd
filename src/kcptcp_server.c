...
int main(int argc, char **argv) {
    ...
    if (bind(usock, &cfg.laddr.sa, (socklen_t)sizeof_sockaddr(&cfg.laddr)) <
        0) {
            ...
            if (tag == &magic_listener) {
                ...
                for (;;) {
                    ...
                    if (rn >= 2 &&
                        ...
                        int ts = socket(cfg.taddr.sa.sa_family,
                        ...
                        if (cfg.tcp_nodelay) {
                            int one = 1;
                            (void)setsockopt(ts, IPPROTO_TCP, TCP_NODELAY, &one,
                                             sizeof(one));
                        }
                        int cr =
                        ...
                        tev.events = EPOLLIN | EPOLLOUT | EPOLLRDHUP |
                                     EPOLLERR | EPOLLHUP;
                        tev.data.ptr = nc;
                    ...
                    for (;;) {
                        ...
                        if (c->state != S_FORWARDING) {
                            ...
                            /* Ensure EPOLLOUT is enabled to both complete
                             * connect and flush later */
                            struct epoll_event tev2 = (struct epoll_event){0};
                            tev2.events = EPOLLIN | EPOLLOUT | EPOLLRDHUP |
                                          EPOLLERR | EPOLLHUP;
                            tev2.data.ptr = c;
