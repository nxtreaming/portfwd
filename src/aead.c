...
int aead_verify_packet(struct proxy_conn *c, uint8_t *data, int len, uint32_t *out_seq) {
    ...
    *out_seq = seq;
    ...
    if (chacha20poly1305_open(c->session_key, nonce, ad, 5, payload, plen, data + 1 + 4 + plen, data) != 0) {
        return -1;
    }
    return plen;
}
...
void aead_seal_packet(struct proxy_conn *c, uint32_t seq, const uint8_t *payload, size_t plen, uint8_t *out_pkt) {
    ...
    chacha20poly1305_seal(c->session_key, nonce, ad, 5, payload, plen, out_pkt + 1 + 4, out_pkt + 1 + 4 + plen);
}
