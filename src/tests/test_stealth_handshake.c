...
int test_stealth_handshake_full_flow() {
    ...
    if (stealth_handshake_create_first_packet(psk, client_token, (const uint8_t *)initial_request,
                                              request_len, client_packet, &client_packet_len) != 0) {
    ...
    if (stealth_handshake_create_response(psk, server_conv, server_payload.token,
    ...
    if (stealth_handshake_parse_response(psk, server_response, server_response_len, &client_response) != 0) {
...
int test_stealth_handshake_wrong_psk() {
    ...
    if (stealth_handshake_create_first_packet(correct_psk, token, NULL, 0, packet, &packet_len) != 0) {
...
int main() {
    ...
    printf("\nTest Results:\n");
    if (failures == 0) {
        printf("✓ All stealth handshake tests passed! Protocol is working correctly.\n");
        return 0;
    } else {
