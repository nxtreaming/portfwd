...
/* Find maximum floating value after key, tolerant to following text like "
 * Mbps" */
...
/* Find the maximum numeric value appearing after a given key pattern, e.g.,
 * key="rekey i=" */
...
/* Return true if needle appears as a contiguous substring within the first len
 * bytes of buf */
...
static void fill_pattern(unsigned char *b, size_t n, unsigned seed) {
    for (size_t i = 0; i < n; i++)
        b[i] = (unsigned char)((i * 1315423911u + seed) & 0xff);
}
...
int main(int argc, char **argv) {
    ...
    // Launch kcptcp-server: 0.0.0.0:4000 -> 127.0.0.1:2323 with -K (capture
    // logs)
    char *srv_argv[] = {(char *)"../kcptcp-server",
                        (char *)"0.0.0.0:4000",
                        (char *)"127.0.0.1:2323",
                        (char *)"-K",
                        (char *)psk,
    ...
    // Launch kcptcp-client: 127.0.0.1:2023 -> 127.0.0.1:4000 with -K (capture
    // logs)
    char *cli_argv[] = {(char *)"../kcptcp-client",
                        (char *)"127.0.0.1:2023",
                        (char *)"127.0.0.1:4000",
                        (char *)"-K",
                        (char *)psk,
    ...
    while (sent < total && !g_stop) {
        ...
        while (roff < n) {
            ...
            if (rn == 0) {
                ...
                die("peer closed early at %zu/%zu", recvd, total);
            }
            if (rn < 0) {
                if (errno == EINTR)
                    continue;
        ...
        if (memcmp(buf, rcv, n) != 0)
            die("mismatch in echo data");
        recvd += n;
    }
...
out:
    ...
    if (mode_timeout) {
        if (!timeout_observed)
            die("expected timeout was NOT observed");
        ...
        if (!has_cli && !has_svr)
            die("missing timeout log in client/server output");
        fprintf(stderr, "[it] timeout observed successfully.\n");
    } else {
        if (recvd != total)
            die("incomplete transfer: %zu/%zu", recvd, total);
        // Check logs for rekey lifecycle: trigger, recv REKEY_ACK, epoch switch
        read_all_fd(cli.fd, &cli_log, &cli_len);
        read_all_fd(srv.fd, &srv_log, &srv_len);
        bool ok = true;
        if (!contains(cli_log, cli_len, "rekey trigger"))
            ok = false;
        if (!contains(cli_log, cli_len, "recv REKEY_ACK"))
            ok = false;
        if (!contains(cli_log, cli_len, "epoch switch"))
            ok = false;
        if (!contains(srv_log, srv_len, "recv REKEY_INIT"))
            ok = false;
        if (!contains(srv_log, srv_len, "epoch switch"))
            ok = false;
        ...
        if (!has_stats_cli && !has_stats_srv)
            ok = false;
        ...
        if (!has_total_cli && !has_total_srv)
            ok = false;
        // Validate throughput counters (expect some positive traffic)
        double tcp_in = find_max_double(cli_log, cli_len, "TCP in=");
        if (tcp_in <= 0.0)
            tcp_in = find_max_double(srv_log, srv_len, "TCP in=");
        double kcp_out = find_max_double(cli_log, cli_len, "KCP payload out=");
        if (kcp_out <= 0.0)
            kcp_out = find_max_double(srv_log, srv_len, "KCP payload out=");
        if (tcp_in <= 0.0 && kcp_out <= 0.0)
            ok = false;
        // Validate core counters from totals
        struct totals t = {0};
        bool have_totals = false;
        if (find_last_totals(cli_log, cli_len, &t))
            have_totals = true;
        else if (find_last_totals(srv_log, srv_len, &t))
            have_totals = true;
        if (!have_totals)
            ok = false;
        if (have_totals) {
            if (t.tcp_rx == 0ull || t.tcp_tx == 0ull)
                ok = false;
            if (t.udp_rx == 0ull || t.udp_tx == 0ull)
                ok = false;
            if (t.kcp_rx_bytes == 0ull || t.kcp_tx_bytes == 0ull)
                ok = false;
            if (t.kcp_rx_msgs == 0ull || t.kcp_tx_msgs == 0ull)
                ok = false;
            if (t.rekeys_i == 0u || t.rekeys_c == 0u)
                ok = false;
        }
        // Verify rekey counter deltas observed in stats logs (expect >=1
