 ...
 * 
 * This function inspects the packet type and dispatches to the appropriate handler
 * for AEAD control messages (rekeying) or data decryption.
 * 
 ...
 * @param out_payload Pointer to be updated with the start of the decrypted payload.
 * @param out_plen Pointer to be updated with the length of the decrypted payload.
 * @return int 0 on success (data packet processed), 1 if it was a handled control packet,
 *         -1 on error (e.g., tag verification failed, which should lead to connection closure).
 */
 ...
 * 
 * Handles AEAD wrapping (if enabled) and potential rekeying triggers before sending.
 * 
 * @param c The connection context.
