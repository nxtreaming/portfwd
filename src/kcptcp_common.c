...
int kcptcp_setup_tcp_listener(const union sockaddr_inx *addr, bool reuse_addr,
    ...
    if (fd < 0) return -1;
    if (reuse_addr) { int on = 1; (void)setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on)); }
#ifdef SO_REUSEPORT
    if (reuse_port) { int on = 1; (void)setsockopt(fd, SOL_SOCKET, SO_REUSEPORT, &on, sizeof(on)); }
#endif
#ifdef IPV6_V6ONLY
    if (v6only && addr->sa.sa_family == AF_INET6) { int on = 1; (void)setsockopt(fd, IPPROTO_IPV6, IPV6_V6ONLY, &on, sizeof(on)); }
#endif
    set_nonblock(fd);
    set_sock_buffers_sz(fd, sockbuf_bytes);
    if (bind(fd, &addr->sa, (socklen_t)sizeof_sockaddr(addr)) < 0) { close(fd); return -1; }
...
/* ---------------- Common CLI parsing ---------------- */
int kcptcp_parse_common_opts(int argc, char **argv, struct kcptcp_common_cli *out,
                             int *pos_start, bool is_server) {
    (void)is_server;
    if (!out) return 0;
    ...
    out->has_psk = false;
    out->kcp_mtu = 0;
    out->kcp_nd = -1; out->kcp_it = -1; out->kcp_rs = -1; out->kcp_nc = -1;
    out->kcp_snd = -1; out->kcp_rcv = -1;
    out->hs_agg_min_ms = 20; out->hs_agg_max_ms = 80; out->hs_agg_max_bytes = 1024;
    out->hs_rsp_jitter_min_ms = 5; out->hs_rsp_jitter_max_ms = 20;
    out->hs_profile = NULL;
    out->show_help = false;
    ...
    while ((opt = getopt(argc, argv, "dp:rR6b:NK:M:n:I:X:C:w:W:g:G:j:P:h")) != -1) {
        ...
        case 'X': { long v = strtol(optarg, NULL, 10); if (v < 0) return 0; out->kcp_rs = (int)v; break; }
        case 'C': { long v = strtol(optarg, NULL, 10); if (v < 0) return 0; out->kcp_nc = (int)v; break; }
        case 'w': { long v = strtol(optarg, NULL, 10); if (v < 0) return 0; out->kcp_snd = (int)v; break; }
        case 'W': { long v = strtol(optarg, NULL, 10); if (v < 0) return 0; out->kcp_rcv = (int)v; break; }
        case 'g': {
...
void kcptcp_maybe_log_stats(struct proxy_conn *c, uint64_t now_ms) {
    ...
    if (now_ms - c->last_stat_ms < get_stats_interval_ms()) return;
    ...
    P_LOG_INFO("stats conv=%u: TCP in=%.3f Mbps out=%.3f Mbps | KCP payload in=%.3f Mbps out=%.3f Mbps | KCP xmit_delta=%u RTT=%dms | rekey i=%u c=%u",
               c->conv, tcp_in_mbps, tcp_out_mbps, kcp_in_mbps, kcp_out_mbps,
               d_xmit, c->kcp->rx_srtt, d_rekey_i, d_rekey_c);
    c->last_stat_ms = now_ms;
    c->last_tcp_rx_bytes = c->tcp_rx_bytes;
    c->last_tcp_tx_bytes = c->tcp_tx_bytes;
    c->last_kcp_tx_bytes = c->kcp_tx_bytes;
...
void kcptcp_log_total_stats(struct proxy_conn *c) {
    if (!c) return;
    if (!get_stats_dump_enabled()) return;
    P_LOG_INFO(
        ...
        c->conv,
        (unsigned long long)c->tcp_rx_bytes,
        (unsigned long long)c->tcp_tx_bytes,
        (unsigned long long)c->udp_rx_bytes,
        (unsigned long long)c->udp_tx_bytes,
        (unsigned long long)c->kcp_rx_msgs,
        (unsigned long long)c->kcp_tx_msgs,
        (unsigned long long)c->kcp_rx_bytes,
        (unsigned long long)c->kcp_tx_bytes,
        c->rekeys_initiated,
        c->rekeys_completed);
}
...
int stealth_handshake_create_first_packet(const uint8_t *psk, const uint8_t *token,
    ...
    if (!psk || !token || !out_packet || !out_packet_len) return -1;
    ...
    if (secure_random_bytes((uint8_t *)&payload.nonce, sizeof(payload.nonce)) != 0) return -1;
    if (secure_random_bytes(payload.reserved, sizeof(payload.reserved)) != 0) return -1;

    uint8_t pad_rnd = 0; if (secure_random_bytes(&pad_rnd, 1) != 0) return -1;
    size_t padding_size = 16 + (size_t)(pad_rnd % 32); /* 16-47 */
    size_t total_size = sizeof(payload) + initial_data_len + padding_size;
    if (total_size + 28 > *out_packet_len) return -1; /* +12 nonce +16 tag */

    uint8_t *plaintext = (uint8_t *)malloc(total_size);
    if (!plaintext) return -1;
    memcpy(plaintext, &payload, sizeof(payload));
    if (initial_data && initial_data_len > 0) memcpy(plaintext + sizeof(payload), initial_data, initial_data_len);
    if (secure_random_bytes(plaintext + sizeof(payload) + initial_data_len, padding_size) != 0) { free(plaintext); return -1; }

    uint8_t nonce[12]; if (secure_random_bytes(nonce, sizeof(nonce)) != 0) { free(plaintext); return -1; }
    uint8_t tag[16];
...
int stealth_handshake_parse_first_packet(const uint8_t *psk, const uint8_t *packet, size_t packet_len,
    ...
    if (!psk || !packet || !payload || packet_len < 28) return -1;
    ...
    if (!plaintext) return -1;
    if (chacha20poly1305_open(psk, nonce, NULL, 0, ciphertext, ciphertext_len, tag, plaintext) != 0) { free(plaintext); return -1; }
    if (ciphertext_len < sizeof(struct stealth_handshake_payload)) { free(plaintext); return -1; }
    memcpy(payload, plaintext, sizeof(struct stealth_handshake_payload));
    if (ntohl(payload->magic) != STEALTH_HANDSHAKE_MAGIC) { free(plaintext); return -1; }
    time_t now = time(NULL);
    time_t msg_time = (time_t)ntohl(payload->timestamp);
    if (msg_time < now - 300 || msg_time > now + 300) { free(plaintext); return -1; }
...
int stealth_handshake_create_response(const uint8_t *psk, uint32_t conv, const uint8_t *token,
    ...
    if (secure_random_bytes(response.reserved, sizeof(response.reserved)) != 0) return -1;

    uint8_t pad_rnd = 0; if (secure_random_bytes(&pad_rnd, 1) != 0) return -1;
    size_t padding_size = 8 + (size_t)(pad_rnd % 24);
    size_t total_size = sizeof(response) + padding_size;
    if (total_size + 28 > *out_packet_len) return -1;

    uint8_t *plaintext = (uint8_t *)malloc(total_size);
    if (!plaintext) return -1;
    memcpy(plaintext, &response, sizeof(response));
    if (secure_random_bytes(plaintext + sizeof(response), padding_size) != 0) { free(plaintext); return -1; }
...
int stealth_handshake_parse_response(const uint8_t *psk, const uint8_t *packet, size_t packet_len,
                                     struct stealth_handshake_response *response) {
    if (!psk || !packet || !response || packet_len < 28) return -1;
    const uint8_t *nonce = packet;
    const uint8_t *ciphertext = packet + 12;
    size_t ciphertext_len = packet_len - 28;
    const uint8_t *tag = packet + 12 + ciphertext_len;
    ...
    if (!plaintext) return -1;
    if (chacha20poly1305_open(psk, nonce, NULL, 0, ciphertext, ciphertext_len, tag, plaintext) != 0) { free(plaintext); return -1; }
    if (ciphertext_len < sizeof(struct stealth_handshake_response)) { free(plaintext); return -1; }

    memcpy(response, plaintext, sizeof(struct stealth_handshake_response));
    if (ntohl(response->magic) != STEALTH_RESPONSE_MAGIC) { free(plaintext); return -1; }
    time_t now = time(NULL);
...
uint32_t kcptcp_stealth_embed_cap_from_mtu(int mtu) {
    ...
    if ((size_t)mtu <= overhead) return 0;
