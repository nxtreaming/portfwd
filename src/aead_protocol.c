...
static int send_rekey_init(struct proxy_conn *c, const uint8_t *psk) {
    ...
    if (derive_session_key_epoch(psk, c->hs_token, c->conv, c->next_epoch, c->next_session_key) != 0) {
        return -1;
    }
...
int aead_protocol_handle_incoming_packet(struct proxy_conn *c, char *data, int len, const uint8_t *psk, bool has_psk, char **out_payload, int *out_plen) {
    if (len < 1) return 0;
    ...
    *out_plen = 0;
    ...
    if (has_psk && (t == KTP_DATA || t == KTP_FIN)) {
        P_LOG_ERR("Plaintext packet type %d in encrypted session conv=%u", (int)t, c->conv);
        return -1;
    }
    ...
    if (verified < 0) {
        P_LOG_WARN("Packet verification failed for type %d, len %d", (int)t, len);
...
int aead_protocol_send_data(struct proxy_conn *c, const char *data, int len, const uint8_t *psk, bool has_psk) {
    if (!has_psk) {
