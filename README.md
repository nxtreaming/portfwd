...
### udpfwd
    ...
    Options:
      ...
      -H <size>          hash table size for UDP connection tracking (default: 128)
      -p <pidfile>       write PID to file
      -h                 show help
...
- UDP_PROXY_BATCH_SZ: Linux-only `recvmmsg()` batch size (default 32)
- UDP_PROXY_DGRAM_CAP: per-datagram buffer capacity (default 1472)
- UDP_PROXY_MAX_CONNS: maximum tracked UDP connections (default 64, optimized for ≤32 concurrent users)
